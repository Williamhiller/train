...
class V2BettingSimulator:
    def __init__(self, data_root, model_dir, model_version="2.0.2", model_name="xgboost", data_season="2025-2026"):
    ...
    def load_odds_data(self, test_df):
        ...
        for feature in self.features:
            if feature not in test_df.columns:
                ...
                elif 'change' in feature or 'trend' in feature:
                    # 赔率变化相关特征，使用默认值0
                    test_df[feature] = 0.0
                elif 'home_' in feature or 'away_' in feature or 'head_to_head' in feature:
                    # 球队状态相关特征，使用默认值0
                    test_df[feature] = 0.0
                else:
                    # 其他特征使用默认值0
    ...
    def simulate_betting(self, base_bet_amount=10, use_fixed_amount=True):
        """模拟投注过程"""
        print(f"\n=== 开始模拟投注 ===")
        print(f"赛季: {self.data_season}")
        print(f"模型版本: v{self.model_version}")
        print(f"模型类型: {self.model_name} v2版本")
    ...
    def generate_report(self, simulation_result):
        ...
        # 保存为JSON格式
        result_json_path = os.path.join(output_dir, f"betting_result_v{self.model_version}.json")
        with open(result_json_path, 'w', encoding='utf-8') as f:
            json.dump(simulation_result, f, ensure_ascii=False, indent=2)
        
        # 保存为CSV格式，方便查看
        bet_df = pd.DataFrame(bet_history)
        result_csv_path = os.path.join(output_dir, f"betting_result_v{self.model_version}.csv")
    ...
    def run_simulation(self, bet_amount=10, use_fixed_amount=True):
        """运行完整的投注模拟"""
