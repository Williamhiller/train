...
class SmartExpertFeatureExtractor(ExpertFeatureExtractor):
    ...
    def _load_or_train_rules_model(self):
        """
        加载或训练规则模型
        """
        # 尝试加载已保存的规则权重
        if os.path.exists(self.weights_path):
            ...
            # 初始化规则权重
            self.rule_weights = {rule_text: 0.7 for rule_text in self.extracted_rules}
            # 保存初始化权重
            self._save_rule_weights()

    def _save_rule_weights(self):
        """
        保存规则权重
        """
        ...
        with open(self.weights_path, 'w', encoding='utf-8') as f:
            json.dump(self.rule_weights, f, ensure_ascii=False, indent=2)
    ...
    def _parse_conclusion(self, conclusion_text: str) -> Dict[str, Any]:
        """
        解析结论文本为结构化结论
        ...
        # 提取预测结果
        if '主胜' in conclusion_text:
            conclusion['result'] = '主胜'
        elif '平局' in conclusion_text:
            conclusion['result'] = '平局'
        elif '客胜' in conclusion_text or '负' in conclusion_text:
            conclusion['result'] = '客胜'
        
        # 根据关键词确定置信度
        if '极高' in conclusion_text or '极大' in conclusion_text:
            conclusion['confidence'] = 0.9
            conclusion['is_strong'] = True
        elif '较高' in conclusion_text or '较大' in conclusion_text:
            conclusion['confidence'] = 0.7
        elif '较低' in conclusion_text or '较小' in conclusion_text:
            conclusion['confidence'] = 0.3
        elif '极低' in conclusion_text:
            conclusion['confidence'] = 0.1
            conclusion['is_strong'] = True
    ...
    def _match_recent_record_condition(self, condition: Dict[str, Any], team_state_data: Dict[str, Any]) -> float:
        ...
        # 获取主/客队或两队的近期战绩
        if team == '主队':
            recent_data = team_state_data.get('home_recent_form', {})
        elif team == '客队':
            recent_data = team_state_data.get('away_recent_form', {})
        else:  # 两队
            home_recent = team_state_data.get('home_recent_form', {})
            away_recent = team_state_data.get('away_recent_form', {})
            # 计算两队的平均战绩
            recent_data = {
                'recent_matches': (home_recent.get('recent_matches', 0) + away_recent.get('recent_matches', 0)) / 2,
                'recent_wins': (home_recent.get('recent_wins', 0) + away_recent.get('recent_wins', 0)) / 2,
                'recent_draws': (home_recent.get('recent_draws', 0) + away_recent.get('recent_draws', 0)) / 2,
                'recent_losses': (home_recent.get('recent_losses', 0) + away_recent.get('recent_losses', 0)) / 2,
                'win_rate': (home_recent.get('win_rate', 0) + away_recent.get('win_rate', 0)) / 2
            }
        
        # 归一化比赛数量
        if recent_data.get('recent_matches', 0) == 0:
            return 0.0
        
        actual_matches = recent_data['recent_matches']
        actual_wins = recent_data['recent_wins']
        actual_draws = recent_data['recent_draws']
        actual_losses = recent_data['recent_losses']
        
        # 将目标战绩按比例缩放到实际比赛数量
        if target_total > 0:
            scale_factor = actual_matches / target_total
        else:
            scale_factor = 1.0
        ...
        # 计算胜率匹配度（使用线性插值）
        win_diff = abs(actual_wins - scaled_target_wins)
        draw_diff = abs(actual_draws - scaled_target_draws)
        loss_diff = abs(actual_losses - scaled_target_losses)
        
        # 计算最大可能差值（用于归一化）
        max_diff = max(actual_matches, scaled_target_wins + scaled_target_draws + scaled_target_losses)
        
        if max_diff == 0:
            return 1.0
        
        # 计算各项匹配度（接近1表示匹配度高）
        win_match_degree = 1.0 - (win_diff / max_diff)
        draw_match_degree = 1.0 - (draw_diff / max_diff)
        loss_match_degree = 1.0 - (loss_diff / max_diff)
        
        # 加权平均匹配度（根据目标战绩的比例分配权重）
        if target_total > 0:
            win_weight = target_wins / target_total
            draw_weight = target_draws / target_total
            loss_weight = target_losses / target_total
        else:
            win_weight = draw_weight = loss_weight = 1/3
        
        match_degree = win_match_degree * win_weight + draw_match_degree * draw_weight + loss_match_degree * loss_weight
        
        return max(0.0, min(1.0, match_degree))
    ...
    def _match_win_rate_condition(self, condition: Dict[str, Any], team_state_data: Dict[str, Any]) -> float:
        ...
        if team == '主队':
            if location == '主场':
                ...
                current_rate = team_state_data['home_team'].get('head_to_head', {}).get('win_rate', 0.5)
            else:
                current_rate = team_state_data['home_team']['recent_form']['win_rate']
        elif team == '客队':
            if location == '客场':
                ...
                current_rate = team_state_data['away_team'].get('head_to_head', {}).get('win_rate', 0.5)
            else:
                current_rate = team_state_data['away_team']['recent_form']['win_rate']
        elif team == '两队':
            if rate_type == '平局率' and location == '近期交锋':
                current_rate = team_state_data['home_team'].get('head_to_head', {}).get('draw_rate', 0.5)
            else:
                # 两队平均胜率
    ...
    def _get_bookmaker_odds(self, odds_data: Dict[str, Any], odds_type: str, bookmaker: str) -> Optional[float]:
        ...
        ladbrokes = bookmakers.get('ladbrokes', {})
        closing_odds = ladbrokes.get('closing_odds', {})
        return closing_odds.get(odds_type)
    ...
    def _get_representative_odds(self, odds_data: Dict[str, Any]) -> Tuple[float, float, float]:
        ...
        """
        # 从ladbrokes获取终盘赔率作为代表性赔率
        bookmakers = odds_data.get('bookmakers', {})
        ladbrokes = bookmakers.get('ladbrokes', {})
        closing_odds = ladbrokes.get('closing_odds', {})
        
        # 获取主胜、平局、客胜赔率
        win_odds = float(closing_odds.get('win', '1.0'))
        draw_odds = float(closing_odds.get('draw', '3.0'))
        lose_odds = float(closing_odds.get('lose', '3.0'))
        
        return (win_odds, draw_odds, lose_odds)
    
    def _match_rule_to_context(self, rule: Dict[str, Any], odds_data: Dict[str, Any], team_state_data: Dict[str, Any]) -> float:
        """
        将规则与当前比赛上下文匹配
        ...
        for condition in conditions:
            ...
            if condition['type'] == 'recent_record':
                match_score = self._match_recent_record_condition(condition, team_state_data)
            elif condition['type'] == 'odds':
                match_score = self._match_odds_condition(condition, win_odds, draw_odds, lose_odds)
            elif condition['type'] == 'win_rate':
                match_score = self._match_win_rate_condition(condition, team_state_data)
            elif condition['type'] == 'recent_state':
                match_score = self._match_recent_state_condition(condition, team_state_data)
            elif condition['type'] == 'odds_change':
                match_score = self._match_odds_change_condition(condition, odds_data)
            elif condition['type'] == 'odds_diff':
                match_score = self._match_odds_diff_condition(condition, odds_data)
            elif condition['type'] == 'other':
                # 其他条件暂时给较低的匹配度
                match_score = 0.3
            
            condition_matches.append(match_score)
        
        # 计算总体匹配度（使用几何平均，要求所有条件都有一定匹配度）
        if not condition_matches:
            return 0.0
        
        # 检查是否有条件匹配度为0
        if any(score == 0.0 for score in condition_matches):
            return 0.0
        
        # 计算几何平均
        import math
        geometric_mean = math.exp(sum(math.log(score) for score in condition_matches) / len(condition_matches))
        
        return geometric_mean
    def extract_expert_features(self, match_data: Dict[str, Any], odds_data: Dict[str, Any], team_state_data: Dict[str, Any]) -> Dict[str, float]:
        """
        提取专家特征（改进版）
        ...
        """
        # 获取基础专家特征
        expert_features = super().extract_expert_features(match_data, odds_data, team_state_data)
    ...
    def _calculate_confidence_score(self, expert_features: Dict[str, float], odds_data: Dict[str, Any], 
        ...
        # 考虑历史交锋记录
        head_to_head = team_state_data['home_team'].get('head_to_head', {})
        if head_to_head.get('total_matches', 0) > 5:
            # 如果历史交锋记录丰富，增加信心
    ...
    def _is_expert_analysis_relevant(self, odds_data: Dict[str, Any], team_state_data: Dict[str, Any]) -> Tuple[bool, float]:
        ...
        relevance_features.append(1.0 - min(state_diff / 0.5, 1.0))  # 状态越接近越相关
        
        # 特征2: 赔率合理性
        expected_home_win_rate = max(0.3, min(0.7, 1.0 / win_odds * 0.8))
        odds_state_match = 1.0 - abs(expected_home_win_rate - home_win_rate)
        relevance_features.append(odds_state_match)
        
        # 特征3: 赔率范围合理性
        is_reasonable_odds = win_odds < 5.0 and lose_odds < 5.0 and abs(win_odds - lose_odds) > 0.3
        relevance_features.append(1.0 if is_reasonable_odds else 0.3)
        
        # 特征4: 有相关的专家规则
        relevant_rules_count = 0
        for rule in self.extracted_rules:
            match_score = self._match_rule_to_context(rule, odds_data, team_state_data)
            if match_score > 0.2:
                relevant_rules_count += 1
        rules_relevance = min(relevant_rules_count / 3.0, 1.0)  # 最多考虑3个规则
        relevance_features.append(rules_relevance)
        
        # 计算综合相关性评分
        weights = [0.25, 0.3, 0.2, 0.25]  # 特征权重
        relevance_score = sum(f * w for f, w in zip(relevance_features, weights))
    ...
    def evaluate_rules_performance(self, match_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        评估所有规则的性能，计算各种指标
        
        参数:
            match_results: 包含比赛结果和规则表现的列表
        ...
        # 按规则分组统计
        rule_performance = defaultdict(lambda: {'correct': 0, 'total': 0, 'matches': []})
        
        for result in match_results:
            if 'applied_rules' in result:
                for rule_text, applied_result in result['applied_rules'].items():
                    rule_performance[rule_text]['total'] += 1
                    if applied_result == result['actual_result']:
                        rule_performance[rule_text]['correct'] += 1
                        all_correct += 1
                    all_total += 1
                    rule_performance[rule_text]['matches'].append({
                        'match_id': result['match_id'],
                        'predicted': applied_result,
                        'actual': result['actual_result'],
                        'correct': applied_result == result['actual_result'],
                        'match_score': result.get('rule_match_scores', {}).get(rule_text, 0.0),
                        'weight': result.get('rule_weights', {}).get(rule_text, 1.0),
                        'timestamp': result.get('timestamp', None)
                    })
        
        # 计算每个规则的指标
        for rule_text, performance in rule_performance.items():
            accuracy = performance['correct'] / performance['total'] if performance['total'] > 0 else 0.5
            precision = performance['correct'] / performance['total'] if performance['total'] > 0 else 0.5
            recall = performance['correct'] / len(match_results) if len(match_results) > 0 else 0.0
            f1 = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0.0
            
            # 计算匹配度与准确率的相关性
            match_scores = [m['match_score'] for m in performance['matches']]
            correct_flags = [m['correct'] for m in performance['matches']]
            correlation = 0.0
            if len(match_scores) > 1:
                correlation = np.corrcoef(match_scores, correct_flags)[0, 1]
            
            # 计算加权准确率
            weighted_scores = [m['match_score'] * m['weight'] for m in performance['matches']]
            weighted_accuracy = sum(ws for ws, c in zip(weighted_scores, correct_flags) if c) / sum(weighted_scores) if sum(weighted_scores) > 0 else 0.5
            
            rule_obj = next((r for r in self.structured_rules if r['raw_text'] == rule_text), None)
            rule_type = rule_obj['type'] if rule_obj else 'unknown'
            conditions_count = len(rule_obj['conditions']) if rule_obj else 0
        ...
        # 计算整体性能指标
        overall_accuracy = all_correct / all_total if all_total > 0 else 0.5
        
        # 按规则类型分组统计
        type_metrics = defaultdict(lambda: {'correct': 0, 'total': 0, 'rules': []})
        for rule_text, metrics in rule_metrics.items():
            rule_type = metrics['type']
            type_metrics[rule_type]['correct'] += metrics['correct']
            type_metrics[rule_type]['total'] += metrics['total']
            type_metrics[rule_type]['rules'].append(rule_text)
        
        for rule_type, metrics in type_metrics.items():
            if metrics['total'] > 0:
                type_metrics[rule_type]['accuracy'] = metrics['correct'] / metrics['total']
                type_metrics[rule_type]['rules_count'] = len(metrics['rules'])
            else:
                type_metrics[rule_type]['accuracy'] = 0.5
                type_metrics[rule_type]['rules_count'] = 0
        ...
        return {
            ...
            'total_rule_applications': all_total
        }

if __name__ == "__main__":
    # 测试专家特征提取器
    extractor = SmartExpertFeatureExtractor()
    print(f"已从PDF中提取 {len(extractor.extracted_rules)} 条专家规则")
    print(f"结构化规则数量: {len(extractor.structured_rules)}")
    
    # 打印前5条规则示例
    print("前5条规则示例:")
    for rule in extractor.structured_rules[:5]:
        result_type = rule['conclusion'].get('result_type', 'unknown')
        confidence = rule['conclusion'].get('confidence', 0.7)
        print(f"  - ID: {rule['id']}")
        print(f"    类型: {rule['type']}")
        print(f"    条件数量: {len(rule['conditions'])}")
        print(f"    结果类型: {result_type}")
        print(f"    置信度: {confidence}")
        print(f"    权重: {extractor.rule_weights[rule['raw_text']]:.4f}")
        print("")
    
    # 打印包含战绩格式的规则
    print("\n包含战绩格式的规则示例:")
    for rule in extractor.structured_rules:
        if rule['id'] in ['rule_16', 'rule_17', 'rule_18', 'rule_19', 'rule_20']:
            print(f"\n  - ID: {rule['id']}")
            print(f"    原规则: {extractor.extracted_rules[int(rule['id'].split('_')[1])-1]}")
            print(f"    结构化条件: {rule['conditions']}")
            print(f"    结论: {rule['conclusion']}")
    
    print("\n\n规则模型已加载，当前规则数量: {}".format(len(extractor.rule_weights)))