...
class BettingSimulator:
    def __init__(self, data_root, model_dir, model_version=3, model_name='xgboost'):
    ...
    def simulate_betting(self, test_data, bet_amount=100, max_bets=100):
        """模拟投注过程"""
        # 特征和标签分离
        ...
        for i in range(min(len(test_data), max_bets)):
            ...
            bet_history.append({
                ...
                'predicted_probability': float(predicted_proba[bet_result]),
    ...
    def run_simulation(self, seasons=['2024-2025'], bet_amount=100, max_bets=100):
        """运行完整的投注模拟"""
        # 准备测试数据
        test_data = self.prepare_test_data(seasons)
        
        # 模拟投注
        simulation_result = self.simulate_betting(test_data, bet_amount, max_bets)
        
        # 输出结果
        self.print_simulation_result(simulation_result)
        
        return simulation_result
    
    def print_simulation_result(self, simulation_result):
        """打印模拟结果"""
        print("=" * 60)
        print("模拟投注结果")
        print("=" * 60)
        print(f"模型版本: V{self.model_version} ({self.model_name})")
        print(f"投注次数: {simulation_result['total_bets']}")
        print(f"每次投注金额: ¥{simulation_result['total_winnings'] / simulation_result['total_bets']:.2f}")
...
def main():
    ...
    # 创建投注模拟器
    simulator = BettingSimulator(data_root, model_dir, model_version=3, model_name='xgboost')
    ...
    # 保存模拟结果
    result_filename = f"betting_simulation_v{simulator.model_version}_{simulator.model_name}.json"
