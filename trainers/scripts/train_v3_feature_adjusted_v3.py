...
# -*- coding: utf-8 -*-
"""
V3模型特征调整训练脚本V3 (版本3.0.5)
- 使用与原始模型相同的特征集（包含team_state和expert特征）
- 改进特征调整策略，使用更精细的调整系数
- 支持分别训练10个赛季和所有数据一起训练
...
class ImprovedFeatureAdjustedModelTrainerV3(ModelTrainerV3):
    """改进的带有特征调整功能的模型训练器V3"""
    ...
    def _load_adjust_features(self):
        ...
        for _, row in df.iterrows():
            if row['importance_diff'] < -0.005 and row['importance_main'] > 0:
                # 改进的调整系数计算：使用更精细的调整策略
                # 1. 基于特征重要性差异的基础调整
                # 2. 确保调整系数在合理范围内（0.8-1.0）
                max_importance = max(row['importance_main'], row['importance_draw'])
                
                # 使用更温和、更精细的调整：只调整15%的差异
                importance_adjust = abs(row['importance_diff']) / max_importance * 0.15
                
                # 基础调整系数
                adjust_coef = 1 - importance_adjust
                
                # 确保调整系数在更合理的范围内
                adjust_coef = max(0.8, min(1.0, adjust_coef))  # 0.8-1.0之间
    ...
    def train(self, seasons, model_name='xgboost', include_team_state=True, include_expert=True, 
        ...
        if custom_version:
            ...
            version = f"3.0.5_feature_adjusted"
...
def train_with_improved_feature_adjustment():
    ...
    # 配置优化策略
    # 策略1: 针对平局的自定义权重
    class_weight = {0: 1.0, 1: 1.8, 2: 1.0}  # 平局权重设为1.8倍
    
    # 策略2: 调整分类阈值，降低平局阈值
    thresholds = [0.35, 0.28, 0.32]  # 客胜:0.35, 平局:0.28, 主胜:0.32
    
    print("=== V3模型改进特征调整训练开始 (版本3.0.5) ===")
    ...
    for season in seasons:
        ...
        try:
            # 使用带有特征调整功能的训练器
            trainer = ImprovedFeatureAdjustedModelTrainerV3(data_root, model_dir)
            ...
            model, model_info = trainer.train(
                ...
                include_expert=True,
                custom_version=f"3.0.5_feature_adjusted_{season}",
            ...
            away_win_metrics = classification_rep.get('客胜', {})
            home_win_metrics = classification_rep.get('主胜', {})
            
            results[season] = {
                'accuracy': model_info['accuracy'],
                'away_win_precision': away_win_metrics.get('precision', 0.0),
                'away_win_recall': away_win_metrics.get('recall', 0.0),
                'away_win_f1': away_win_metrics.get('f1-score', 0.0),
                ...
                'home_win_precision': home_win_metrics.get('precision', 0.0),
                'home_win_recall': home_win_metrics.get('recall', 0.0),
                'home_win_f1': home_win_metrics.get('f1-score', 0.0)
            }
            
            print(f"\n{season}赛季模型训练完成！")
            print(f"总准确率: {model_info['accuracy']:.4f}")
            print(f"客胜F1分数: {away_win_metrics.get('f1-score', 0.0):.4f}")
            print(f"平局F1分数: {draw_metrics.get('f1-score', 0.0):.4f}")
            print(f"主胜F1分数: {home_win_metrics.get('f1-score', 0.0):.4f}")
    ...
    try:
        ...
        model, model_info = trainer.train(
            ...
            include_team_state=True,
            include_expert=True,
            custom_version="3.0.5_feature_adjusted_all_seasons",
            class_weight=class_weight,
            thresholds=thresholds
        )
    ...
    # 排除所有赛季模型，只计算单个赛季的平均
    season_results = {k: v for k, v in results.items() if k != 'all_seasons'}
    if season_results:
        avg_accuracy = np.mean([r['accuracy'] for r in season_results.values()])
        avg_away_win_f1 = np.mean([r['away_win_f1'] for r in season_results.values()])
        avg_draw_f1 = np.mean([r['draw_f1'] for r in season_results.values()])
        avg_home_win_f1 = np.mean([r['home_win_f1'] for r in season_results.values()])
        
        print(f"单个赛季模型平均准确率: {avg_accuracy:.4f}")
        print(f"单个赛季模型平均客胜F1分数: {avg_away_win_f1:.4f}")
        print(f"单个赛季模型平均平局F1分数: {avg_draw_f1:.4f}")
        print(f"单个赛季模型平均主胜F1分数: {avg_home_win_f1:.4f}")
    
    # 打印所有赛季模型的结果
    if 'all_seasons' in results:
        all_results = results['all_seasons']
        print(f"\n所有赛季数据模型准确率: {all_results['accuracy']:.4f}")
        print(f"所有赛季数据模型客胜F1分数: {all_results['away_win_f1']:.4f}")
        print(f"所有赛季数据模型平局F1分数: {all_results['draw_f1']:.4f}")
        print(f"所有赛季数据模型主胜F1分数: {all_results['home_win_f1']:.4f}")
    
    # 保存结果
    results_file = '/Users/Williamhiler/Documents/my-project/train/feature_adjusted_results_v3.json'
